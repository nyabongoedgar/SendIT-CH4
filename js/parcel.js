...
const url = "http://sendit299.herokuapp.com/api/v2/parcels";
...
function create_parcel(){  
    ...
    if (token){
        ...
        .then(resp_data =>{
            return window.location.assign('http://sendit300.herokuapp.com/users/all-orders.html');
            })
        .catch(error => {
             console.log(error); 
            });
    ...
    function change_parcel_destination(){
        ...
        for (var i = 1; i < rows.length; i++){
            rows[i].onclick = function(){
                ...
                const url_destination = "http://sendit299.herokuapp.com/api/v2/parcels/"+parcelId+"/destination";
